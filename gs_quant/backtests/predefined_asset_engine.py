...
class PredefinedAssetEngine(BacktestBaseEngine):
    ...
    def _timer(self, strategy, start, end, frequency, states=None):
        dates = (
            list(map(lambda x: x.date(), pd.to_datetime(pd.bdate_range(start=start, end=end, freq=frequency))))
            if states is None
            else states
        )
        ...
        if self.calendars is not None:
            dates = list(
                compress(
                    dates, is_business_day(dates, (None if self.calendars.lower() == 'weekend' else self.calendars))
                )
            )
    ...
    def run_backtest(self, strategy, start, end, frequency="B", states=None, initial_value=100):
        ...
        if states is not None:
            ...
            # create timer
            timer_start = (
                (adjusted_start + BDay(1)).date()
                if self.calendars is None
                else business_day_offset(
                    adjusted_start,
                    1,
                    roll='forward',
