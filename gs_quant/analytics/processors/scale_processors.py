...
class SpotMarkerProcessor(BaseProcessor):
    def __init__(self, a: BaseProcessor, *, name: str, shape: ScaleShape):
        """SpotMarkerProcessor
        ...
        if isinstance(a_data, ProcessorResult):
            if a_data.success:
                self.value = ProcessorResult(
                    True, {'name': self.name, 'value': a_data.data.get(-1), 'shape': self.shape.value}
                )
            else:
...
class BarMarkerProcessor(BaseProcessor):
    def __init__(self, start: BaseProcessor, end: BaseProcessor, *, name: str):
        """BarMarkerProcessor
    ...
    def process(self):
        ...
        if isinstance(start, ProcessorResult) and isinstance(end, ProcessorResult):
            if start.success and end.success:
                self.value = ProcessorResult(
                    True,
                    {'name': self.name, 'start': start.data.get(0), 'end': end.data.get(0), 'shape': self.shape.value},
                )
            else:
...
def validate_markers_data(result: Dict, marker_data: Dict) -> Tuple[bool, str]:
    ...
    if ScaleShape(marker_data['shape']) == ScaleShape.BAR:
        ...
        if starting_val > ending_val or starting_val < min_val or starting_val > max_val:
            return (
                False,
                f'Invalid marker={marker_name} with starting value={starting_val}.'
        ...
        if ending_val < starting_val or ending_val < min_val or ending_val > max_val:
            return (
                False,
                f'Invalid marker={marker_name} with ending value={ending_val}.'
        ...
        if marker_data['value'] < min_val or marker_data['value'] > max_val:
            return (
                False,
                f'Invalid marker={marker_data["name"]} with value={marker_data["value"]}. Has to be '
    ...
    def process(self):
        ...
        if isinstance(min_data, ProcessorResult) and isinstance(max_data, ProcessorResult):
            if min_data.success and max_data.success:
                result = {'min': min_data.data.get(0), 'max': max_data.data.get(0), 'markers': []}
