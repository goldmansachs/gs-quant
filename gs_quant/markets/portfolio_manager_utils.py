...
def build_macro_portfolio_exposure_df(
    ...
    if factor_category_dict:
        ...
        universe_sensitivities_df.columns = pd.MultiIndex.from_tuples(
            map(
                lambda factor: (factor_category_dict.get(factor, "Asset Information"), factor),
                universe_sensitivities_df.columns,
            ),
            names=("Macro Factor Category", "Macro Factor"),
        )
        # Aggregate columns to get portfolio total exposure to each macro factor
        sum_over_factors_df = (
            universe_sensitivities_df.loc[:, universe_sensitivities_df.columns != ("Asset Information", "Asset Name")]
            .agg(np.sum, axis="index")
            .to_frame()
            .transpose()
        )
        ...
        sum_over_categories_df = (
            sum_over_factors_df.transpose().groupby("Macro Factor Category").apply(np.sum).transpose()
        )
        factor_and_category_zip = dict(
            zip(portfolio_exposure_df.columns.get_level_values(1), portfolio_exposure_df.columns.get_level_values(0))
        )
        ...
        portfolio_exposure_df = pd.concat(
            [
                ...
                pd.DataFrame(
                    portfolio_exposure_per_category,
                    index=portfolio_exposure_df.columns.tolist(),
                    columns=["Portfolio Exposure Per Macro Factor Category"],
                ).transpose(),
            ]
        )
        ...
        portfolio_exposure_df.loc[
            "Portfolio Exposure Per Macro Factor Category", ("Asset Information", "Asset Name")
        ] = np.nan
        ...
        sum_over_factors_df = (
            universe_sensitivities_df.loc[:, universe_sensitivities_df.columns != 'Asset Name']
            .agg(np.sum, axis="index")
            .to_frame()
            .transpose()
        )
...
def build_portfolio_constituents_df(performance_report: PerformanceReport, date: dt.date) -> pd.DataFrame:
    constituents_df = performance_report.get_portfolio_constituents(
        fields=['netExposure'], start_date=date, end_date=date
    )
    if constituents_df.empty:
        raise MqValueError(
            f"Macro Exposure can't be calculated as the portfolio constituents could not be found on"
    ...
    constituents_df = constituents_df.dropna().set_index("assetId").rename_axis("Asset Identifier")
    assets_data = GsAssetApi.get_many_assets_data_scroll(
        fields=['name', 'gsid', 'id'],
    ...
    assets_data_df = (
        pd.DataFrame.from_records(assets_data)
        .set_index("id")
    ...
    constituents_and_notional_df = (
        assets_data_df.merge(constituents_df, on='Asset Identifier')
...
def build_sensitivity_df(
    ...
    universe_sensitivities_df = model.get_universe_sensitivity(
        start_date=date,
        end_date=date,
        assets=DataAssetsRequest(UniverseIdentifierRequest.gsid, universe),
    ...
    universe_sensitivities_df = (
        universe_sensitivities_df.reset_index(level=1, drop=True).rename_axis("Asset Identifier").sort_index()
    )
...
def build_exposure_df(
    ...
    if factor_data.empty:
        ...
        universe_sensitivities_df = pd.concat(
            [
                universe_sensitivities_df,
                universe_sensitivities_df.agg("sum").to_frame().rename(columns={0: "Total Factor Category Exposure"}).T,
            ]
        )
        ...
        notional_df = pd.concat(
            [
                notional_df,
                notional_df[["Notional"]].agg("sum").to_frame().rename(columns={0: "Total Factor Category Exposure"}).T,
            ]
        )
        ...
        new_columns = (
            ...
            if by_name
            else [(factor_data.loc[f, 'factorCategoryId'], f) for f in universe_sensitivities_df.columns.values]
        )
        universe_sensitivities_df = universe_sensitivities_df.set_axis(
            pd.MultiIndex.from_tuples(new_columns), axis=1
        ).rename_axis(("Factor Category", "Factor"), axis=1)
        universe_sensitivities_df = pd.concat(
            [
                universe_sensitivities_df,
                universe_sensitivities_df.agg("sum").to_frame().rename(columns={0: "Total Factor Exposure"}).T,
            ]
        )
        universe_sensitivities_df = universe_sensitivities_df.sort_values(
        ...
        notional_df = pd.concat(
            ...
            pd.MultiIndex.from_tuples([("Asset Information", "Asset Name"), ("Asset Information", "Notional")]), axis=1
        )
