...
@pytest.mark.parametrize(
    'keys, expected_type, expected_first_key',
    [
        (['2024-06-11', '2024-06-12'], RiskResultsByDate, dt.date(2024, 6, 11)),
        (['2024-06-11T10:00:00', '2024-06-11T11:00:00'], RiskResultsByDate, dt.datetime(2024, 6, 11, 10, 0, 0)),
    ],
)
def test_decode_risk_result(keys, expected_type, expected_first_key):
    encoded_series_result = {'index': ('a', 'b', 'c'), 'name': 'test', 'values': (1, 2, 3)}
    risk_result = {
        'refs': {'legId': 'uuid_1', 'riskMeasure': 'uuid_2'},
        'result': {k: {'unit': 'EUR', 'result': encoded_series_result, 'type': 'vector'} for k in keys},
    }
    ...
    assert isinstance(decoded.result[expected_first_key], VectorWithData)
    assert decoded.result[expected_first_key].unit == 'EUR'
    assert_series_equal(
        decoded.result[expected_first_key].result,
