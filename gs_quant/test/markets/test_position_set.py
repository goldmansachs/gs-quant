...
def test_position_resolve_many(mocker):
    unresolved_position_sets = [
        PositionSet(
            ...
            positions=[
                Position(identifier='GS UN', weight=0.5, tags=[PositionTag(name="tag1", value="tagvalue1")]),
                Position(identifier='AAPL UW', weight=0.5, tags=[PositionTag(name="tag2", value="tagvalue2")]),
            ],
        ),
        PositionSet(
    ...
    mocker.patch.object(
        position_set_module, "_get_asset_temporal_xrefs", return_value=(pd.DataFrame(xref_results), "bbid")
    )
    ...
    mocker.patch.object(
        position_set_module, "_get_asset_temporal_xrefs", return_value=(pd.DataFrame([xref_results[1]]), "bbid")
    )
    ...
    expected_position_set = [
        PositionSet(
            ...
            unresolved_positions=[
                Position(identifier='GS UN', name="GS", weight=0.5, tags=[PositionTag(name="tag1", value="tagvalue1")])
            ],
        ),
        PositionSet(
            date=dt.date(2024, 5, 1),
...
def test_position_price_many(mocker, position_sets_with_tags_and_notional, expected_position_pricing_result):
    # All positions on each holding date are priced

    mocker.patch.object(GsPriceApi, "price_many_positions", return_value=expected_position_pricing_result)
    ...
    with pytest.raises(
        MqValueError,
        match="Unable to price positions without position weights and daily reference notional or position quantities",
    ):
        PositionSet.price_many(
            position_sets_with_tags_and_notional, weighting_strategy=PositionSetWeightingStrategy.Quantity
        )

    # Invalid weighting strategies
    with pytest.raises(MqValueError, match="Can only specify a weighting strategy of weight, notional, or quantity"):
        PositionSet.price_many(
            position_sets_with_tags_and_notional, weighting_strategy=PositionSetWeightingStrategy.Market_Capitalization
        )

    # There are unpriced results
    mocker.patch.object(GsPriceApi, "price_many_positions", return_value=[expected_position_pricing_result[1]])
    new_position_set_list = deepcopy(position_sets_with_tags_and_notional)
    PositionSet.price_many(new_position_set_list)
    ...
    different_tags_common_asset_id_positions = [
        PositionSet(
            ...
            positions=[
                ...
                Position(identifier='AAPL UW', asset_id="MA4B66MW5E27U9VBB94", name="Apple", weight=0.5, tags=None),
            ],
        ),
        PositionSet(
            date=dt.date(2024, 5, 1),
    ...
    mocker.patch.object(GsPriceApi, "price_many_positions", return_value=same_assets_different_weights_pricing_results)
    ...
    different_tags_common_asset_id_positions = [
        PositionSet(date=dt.date(2024, 4, 30), reference_notional=1000, positions=[]),
        PositionSet(
            date=dt.date(2024, 5, 1),
            reference_notional=1000,
            positions=[
    ...
    mocker.patch.object(
        GsPriceApi, "price_many_positions", return_value=[same_assets_different_weights_pricing_results[1]]
    )
    ...
    mocker.patch.object(GsPriceApi, "price_many_positions", return_value=expected_position_notionals_result)
