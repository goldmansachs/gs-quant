...
def test_registry():
    ...
    for name, mm in registry.items():
        assert name == mm.display_name, 'registry key {} does not match with MultiMeasure display_name {}'.format(
...
def test_no_duplicate_plot_measure_function_names():
    ...
    members: list[tuple[str, MultiMeasure]] = inspect.getmembers(timeseries, lambda o: isinstance(o, MultiMeasure))

    fns: set[Callable] = {
        fn for (_, multi_measure) in members for fns in multi_measure.measure_map.values() for fn in fns
    }
    ...
    dups: dict[str, int] = {fn_name: count for (fn_name, count) in fn_name_count.items() if count > 1}
