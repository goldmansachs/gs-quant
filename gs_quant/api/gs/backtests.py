...
class GsBacktestApi:
        ...
        query_string = urlencode(
            dict(
                filter(
                    ...
                    dict(id=backtest_id, ownerId=owner_id, name=name, mqSymbol=mq_symbol, limit=limit).items(),
                )
            )
        )
    ...
    @classmethod
    def run_backtest(
        cls, backtest: Backtest, correlation_id: str = None, timeout: Optional[int] = DEFAULT_TIMEOUT
    ) -> BacktestResult:
        """
