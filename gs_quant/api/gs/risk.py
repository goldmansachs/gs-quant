    ...
    async def get_results(cls, responses: asyncio.Queue, results: Queue, timeout: Optional[int] = None):
        async def send_to_websocket(ws: client):
            while True:
                ...
                await asyncio.wait_for(ws.send(json.dumps(report_ids)), timeout=timeout)
        ...
        async def read_from_websocket(ws: client):
            ...
            if outstanding_report_ids:
                await asyncio.wait_for(ws.send(json.dumps(outstanding_report_ids)), timeout=timeout)

            while True:
                raw_result = await asyncio.wait_for(ws.recv(), timeout=timeout)
                result_id, status_result_str = raw_result.split(';', 1)
