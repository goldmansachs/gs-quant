...
class FundamentalAdvancedItem(Enum):
    ...
    OPER_LEASE_COMMIT_2YR_5YR = "Operating Lease Commitments 2-5 Years"
    ...
    DEBT_BONDS_SECURE_LIEN1 = "t Lien"
    DEBT_BONDS_SECURE_LIEN3 = "Bonds –Sec. 1st Lien"
...
def factset_estimates(asset: Asset, metric: EstimateItem = EstimateItem.EPS,
    ...
    ds_id = f'FE_{basic}_CONH_{BASIS_TO_DATASET[report_basis]}_GLOBAL'
    ds = Dataset(ds_id)
    ...
    if isinstance(period, int) and report_basis not in [EstimateBasis.NTM, EstimateBasis.STM]:
        ...
        if report_basis in [EstimateBasis.NTM, EstimateBasis.STM]:
            ...
            if isinstance(period.p, int) and period.p not in [1, 2, 3, 4]:
                ...
                raise MqValueError('Please specify the period as an integer between 1 and 4 like FiscalPeriod(2022, 4) '
                                   'for 2022Q4 estimate')
            ...
            if isinstance(period.p, int) and period.p not in [1, 2]:
                ...
                raise MqValueError('Please specify the period as 1 or 2 like FiscalPeriod(2022, 2) for 2022H2 estimate')
        ...
        df = df[(df['feFpEnd'] >= pd.to_datetime(fiscal_period_start).date()) & (
                df['feFpEnd'] <= pd.to_datetime(fiscal_period_end).date())]
        if df.empty:
            raise MqValueError('No Data returned for selected fiscal period')

    df.reset_index(inplace=True)
    df['consEndDate'].fillna(end, inplace=True)
    ...
    series = ExtendedSeries(df[f'fe{column_prefix}{statistic.value}{BASIS_TO_FIELD[report_basis]}'],
...
def factset_fundamentals(asset: Asset,
    ...
    start, end = DataContext.current.start_date, DataContext.current.end_date
    start_new = RelativeDate('-1y', base_date=start).apply_rule()
    basic = 'BASIC'  # todo: change this to 'ADVANCED' if in advanced or advanced derived metrics
    if metric.name in fundamental_basic_derived_dict:  # todo: add check for advanced derived metrics
        derived = '_DER'
    else:
        derived = ''
    if report_format == FundamentalFormat.RESTATED:
        ...
        restated = ''
        restated_col_name = ''
    ds_id = f'FF_{basic}{derived}{restated}_{FF_BASIS_TO_DATASET[report_basis]}_GLOBAL'  # TO DO combine all regions
    ds = Dataset(ds_id)
    ...
    column = ('ff' + metric.name.replace('_', ' ').title().replace(' ', '') +
              restated_col_name + FF_BASIS_TO_FIELD[report_basis])
    df = df[['date', column]]
